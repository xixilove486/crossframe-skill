...
- `history`：历史研究、史料互读、历史事件、历史制度、文明连续史、长时段比较、archive/FOIA backlog。
- `critical`：结构批判文章、批判性长文、意识形态/成本转移/利益链分析。
- `review`：审查一个已有输出。
- `inquiry`：诊断、文章或 review 完成后，用户要求继续追问、深思、反证、补证、迁移、行动边界或自我定位。

如果用户同时要求多个目标，确定一个最终交付物，再安排前置 skill。例如“分析公共议题并写文章”的最终交付物是文章，但前置需要 `public`。

只要用户通过 `crossframe-suite` 进入 CrossFrame 内容任务，默认最终交付物都定为 `essay`。如果任务还需要评审、案例库、组织修复、命题辩论、概念教学、读书研究、历史研究或短答复，先完成对应专项产物，再进入 `crossframe-essay`。默认把 `output_mode` 设为 `full-visible-v5-longform`。`voice_mode` 不再固定为旧声口预设，而是由第 0 节判定的角色和 `topic_sensitivity` 决定。
...
- `claim ledger`；由 `crossframe` 核心层在七闸、机制候选、概念契约和源锚点完整性检查后生成，至少覆盖中心命题、机制候选、高风险概念、行动建议、文章转译和公共定性
- 需要追加的专项 skill
...
- 结构批判文章或用户显式点名 `crossframe-critical`：追加 `../../crossframe-critical/SKILL.md`。
...
1. 结构事实层：`crossframe`
2. 场景专项层：`public` / `org` / `debate` / `notebook` / `history` / `critical` / `casebook` / `teach` / `dialogue`
...
6. 命题台账层：用 `templates/claim-ledger.md` 和 `worksheets/claim-ledger-check.md` 确认中心命题、机制候选、高风险概念、行动建议和文章转译都有 `claim_id`
7. 底稿层：`crossframe-essay` 先生成 `结构洞察底稿`
8. 文章类型层：在底稿之后展示文章类型选择器，用户选择或采用推荐项
...
可选追加极短质量闸摘要时，只能写“生成层自检摘要：已修正 X，待 review 判定”。不得写“质量闸通过 / 完全通过 / substantive_pass”，也不得把 `templates/review-report.md` 作为 suite 默认成文链路的唯一最终输出。

质量闸之后必须进入完成态追问接管：若本轮完整交付已经形成，记录 `post_completion_inquiry_armed=true`，并把下一轮用户输入默认交给 `crossframe-inquiry`。这个状态不要求用户再次显式说“继续追问”；它只在用户明确开启新任务、换主题或退出上文时解除。
