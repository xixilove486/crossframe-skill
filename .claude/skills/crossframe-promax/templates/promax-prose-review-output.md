...
- `beat_id`：与 P9 完全相同。
- `action_ids`：与该 beat 冻结的动作数组逐项、顺序完全相同；每项只能是固定八动作序列中的一个或两个相邻动作。
- `section_ids`：集合与该 beat 的冻结 section IDs 完全相同。
- `evidence_excerpts`：至少一条正文逐字短摘。

正文逐字短摘必须真实存在于当前 essay，每条至少包含 8 个实质字符、最多 240 个字符，并且在正文中唯一出现。不得写总结、改写、占位词、可匹配多处的通用片段或不存在的理想句。每个动作都必须由与其语义直接相关的摘录支撑；包含 `withdrawal_action_boundary` 时，摘录必须同时证明撤回或切换条件以及行动上限或授权边界。

`reality_entry` 的映射必须至少有一条摘录落在 essay 的第一个正文段，`resonant_close` 的映射必须至少有一条摘录落在最后一个正文段。Markdown 标题不算正文段，不能拿中段的“现实”或“最后”措辞替代首末位置约束。映射覆盖全部 beats 后才能通过。

映射顺序必须与 P9 beats 相同；每个动作要能从本 beat 摘录中找到自己的语义证据。八个动作对应摘录在实际正文中的偏移只能向后推进，不能把反方或建议段挪到机制之前，再用一份顺序正确的 P9 掩盖正文乱序。
...
摘录必须专属于所审维度：概念保真须绑定 P9 中每个核心概念的 `reader_explanation` 与锚词；立场一致须同时绑定锁定的中心命题、首选和次选自然句；撤回边界须同时绑定 `withdrawal_text` 与 `action_ceiling_text`；反方摘录不得是否认反方存在的句子。全文泄漏检查由 runtime 直接复核，不能靠一条自称“没有泄漏”的句子通过。
