...
- `stance_projection`：逐项复制 P8 的命题关系与判断强度，并冻结将进入正文的中心判断、撤回条件、行动上限，以及被请求时的首选/次选 ID、类型与自然句。正文不展示内部 ID，只逐字承载自然句。
- `core_concept_ids`：真正改变中心判断、机制竞争、反方或建议的 applied concepts。
- `atlas_only_concept_ids`：其余 applied concepts。
- `core_concept_bindings`：逐一绑定全部核心概念；每个概念选择 2–4 个非通用短锚词、1–3 条完整 `definition`/`allowed_inferences` 原句、至多 3 条具体误用原句，并冻结一条包含权威名与全部锚词的自然 `reader_explanation`。解释句必须逐字进入正文。
- `selected_techniques`：严格使用该 `article_type` 的路由，先按路由顺序记录恰三个核心技法，再按需记录零至两个候选辅助技法；总数不得超过五个，ID 必须来自内置 50 卡索引。
- `reader_beats`：把固定八动作分配给段落任务。每个 beat 只承载一个动作，或合并相邻两个动作；每个 beat 至少绑定一个 claim，并登记与该 claim 相连的机制和证据。

`core_concept_ids` 与 `atlas_only_concept_ids` 必须互斥，并集必须等于全部 applied concepts；`core_concept_bindings` 必须与核心概念一一闭合。所有核心概念、计划机制、claim-bound evidence 与已选技法都必须被 reader beats 承载。所有 applied concepts 的权威定义、邻接关系与误用边界仍由 atlas 精确闭合；正文只承担核心概念的读者转译。
...
正文固定按以下八个 reader action 推进。`reader_beats[].action_ids` 展开后必须与此序列逐项相等；每个动作恰好出现一次。可以在一个 beat 内合并相邻两个动作，不能跨动作合并、倒序、重复或漏项：

1. `reality_entry`：一个可核验事实、具体矛盾、高成本信号或真实处境。
2. `center_thesis`：尽早说清真正要判断什么以及当前结论。
3. `mechanism_progression`：解释为何卡住、反复或产生当前结果，并区分至少两个竞争机制。
4. `same_dimension_comparison`：在同对象、同尺度、同窗口和同评价维度下比较路径与代价。
5. `strongest_counterposition`：重建其最强论据、可成立条件和可观察预测，不攻击稻草人。
6. `explicit_position`：说明为何当前仍选择首位判断或方案，以及次选在什么条件下上升。
7. `withdrawal_action_boundary`：写明什么事实会降档、切换或停止，以及当前允许做到哪里。
8. `resonant_close`：回到现实责任或尚未封死的问题，不用宏大口号代替判断。
...
P9 为该概念冻结的 2–4 个 `reader_anchor_terms` 必须逐字来自 v8 registry 的 `definition` 或 `allowed_inferences`；`source_support_spans` 必须完整等于其中一条权威原句，`source_misuse_spans` 必须完整等于 registry 的具体误用边界。正文必须逐字承载 `reader_explanation`。不得选择只有主题相关性、却不能区分概念语义的通用词；“结构、关系、条件、问题、当前、对象、概念、机制、状态、行动、证据”均不能单独充当锚词。
...
每个通过判断都要给出与该维度相关的正文实际短摘。短摘须含至少 8 个实质字符、在正文中唯一出现，并按其覆盖的句区归一化；不同维度不得切碎同一句现实入口充当多份证明。任一句区最多服务两个通过维度，至少三分之二的通过维度必须形成不同的句区组合。宿主模型风味与审计泄漏属于全文性质，审校者必须检查 essay hash 所绑定的全文；短摘只记录代表性检查锚点，不能代替全文检查。review 必须绑定当前 essay、position 与 output plan 的 SHA-256；陈旧散列、正文中不存在的短摘、漏掉 reader beat 或维度自相矛盾都视为失败。

`required_beat_mappings` 必须逐 beat 复制 P9 冻结的 `action_ids` 顺序，并用语义相关的正文短摘证明每个动作。beat 映射顺序必须与 P9 相同，八动作所指短摘在真实正文中的位置只能向后推进。`reality_entry` 至少一条摘录必须落在第一个正文段，`resonant_close` 至少一条摘录必须落在最后一个正文段；`withdrawal_action_boundary` 必须同时证明可观察的撤回或切换条件，以及行动上限或授权边界。中段出现同类措辞不能替代首末段约束。
