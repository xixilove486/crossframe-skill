...
The checked-in `execution-ready` manifest records 0 product runs and 0 blind grades. Benchmark execution is deferred; committed `results.json` remains `not_run` until all 48 product runs and all 72 raw grade files from three fresh blind graders per case exist and pass hash validation. The builder must fail closed: missing evidence, missing hashes, unexpected aggregate fields, fallback runtimes, or changed raw bytes are errors. It never treats prose summaries or hand-authored totals as authority.

## Legal state machine

The only legal transition is `scaffold -> execution-ready -> ready-for-results-build`:

The checked-in manifest is schema-v2 `execution-ready`. Historical schema-v1 scaffolds remain test fixtures only: the execution transition accepts one only as a fully validated source, and after all 24 schema-v2 bundles and their final packet/tree bindings pass in memory, one atomic replacement writes a schema-v2 `execution-ready` manifest. A schema-v1 manifest is never executable, and an already-schema-v2 scaffold follows the same validation gate.
...
`scenarios.json`, `rubric.json`, `pairing-manifest.json`, the frozen case bundles, this README, the empty raw registry, and the builder are deterministic readiness assets. They are not benchmark performance evidence. Expected-pressure files are audit-only and hidden from products and graders. Frozen case materials must pass outcome-leakage, privacy, and license review before product execution.
...
Forward prediction evidence is governed separately by `../ultra-forward/`. This deterministic readiness contract makes no forward-validation claim.
