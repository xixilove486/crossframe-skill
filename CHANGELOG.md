...
## CrossFrame ProMax 1.0.1 - 2026-07-25
...
- 已有 run-contract schema v1、五角色记录、旧 output plan 与旧 validator 集继续按原合同验证；1.0.1 新运行只生成 v2，不回写既有工件。
- skill 名称与目录保持 `crossframe-promax`，不增加 v8 后缀，也不需要 CrossFrame 家族其他 skill 才能运行。

### Verification

- 新增契约兼容、对抗性成文和完整物化运行测试，覆盖陈旧审校、机器字段泄漏、概念/证据错配、缺少最强反方、长而自然的正文与 v1 回放。
